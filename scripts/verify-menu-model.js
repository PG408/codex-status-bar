...
const checks = [
  ...
  ["Swift hides closed Side Chat rows without deleting state", swift.includes("ChatGPTActivityLogMonitor") && swift.includes("isClosedSideChatSession") && swift.includes("isViewActive(sessionId: sessionIdentity(session)) == false")],
  ["Swift retains a five-minute Side Chat fallback when activity logs are unavailable", swift.includes("sideChatRestingMenuHideAfter") && swift.includes("5 * 60") && swift.includes("isHiddenSideChatMenuSession") && swift.includes('== "Side Chat"')],
